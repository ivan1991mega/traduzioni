...
3. Altrimenti traduci il testo nella lingua di destinazione richiesta, riscrivendolo come una scheda prodotto e-commerce: chiaro, naturale per un madrelingua pescatore (mai una traduzione letterale o goffa), mantenendo esatte le informazioni tecniche (materiali, misure, peso, azione della canna, profondita' di pesca, specie target, ecc.).
4. Il testo di partenza puo' contenere refusi, errori di battitura o termini fuori contesto (es. testi gia' tradotti automaticamente in modo maldestro da un'altra lingua). Se un termine non ha senso nel contesto della pesca ma assomiglia chiaramente a un termine tecnico plausibile (es. "bus" al posto di "bass", il pesce persico black bass), correggilo silenziosamente usando il termine corretto, senza segnalarlo nel testo finale. Se il dubbio non e' chiaramente risolvibile dal contesto, traduci in modo neutro senza inventare dettagli.
5. Rendi il testo tradotto attraente e persuasivo quanto l'originale.
6. Il testo tradotto non deve MAI superare i ${MAX_CHARS} caratteri, spazi inclusi. Se il testo originale e' piu' lungo di quanto stia in ${MAX_CHARS} caratteri, sintetizza dando SEMPRE priorita', in quest'ordine: (1) dati tecnici concreti e numerici (materiali, misure, peso, azione, profondita', potenza, specie target), (2) le caratteristiche distintive uniche del prodotto rispetto a prodotti simili, (3) il tono persuasivo. Se devi tagliare qualcosa per rientrare nel limite, taglia per prima cosa gli aggettivi enfatici ripetuti e le frasi ridondanti, mai i dati tecnici specifici. Per il giapponese conta i caratteri (kanji/kana/punteggiatura), non le parole.
7. Rispondi ESCLUSIVAMENTE con un oggetto JSON valido, senza markdown, backtick o testo aggiuntivo, in uno di questi due formati:
...
// Se anche dopo i tentativi il modello principale resta sovraccarico, prova
// una sola volta con un modello di riserva prima di arrendersi.
async function callClaudeWithRetry(params) {
  ...
  if ((lastStatus === 529 || lastStatus === 429) && params.model !== FALLBACK_MODEL) {
    console.warn(`Modello ${params.model} ancora sovraccarico dopo ${MAX_RETRIES} tentativi, provo con ${FALLBACK_MODEL}`);
    try {
...
app.get('/api/health', (req, res) => {
  res.json({ ok: true, model: MODEL, fallbackModel: FALLBACK_MODEL, supportedLangs: SUPPORTED_LANGS });
});
